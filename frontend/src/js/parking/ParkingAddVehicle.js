...
const ParkingAddVehicles = ({ parkingId }) => {
    ...
    useEffect(() =>{
        if (!isNaN(parseInt(pageFromUrl, 10))) {
            ...
            history.replace(`/parking/${parkingId}/add-vehicles?page=1`);
            setPage(1);
        }
    },[pageFromUrl])
    console.log(page);
    ...
    const [sortBy, setSortBy] = useState(localStorage.getItem('parkingAddVehiclesSortBy' + parkingId.toString()) || 'fabricationYear');

    const [order, setOrder] = useState(localStorage.getItem('parkingAddVehiclesOrder' + parkingId.toString()) || 'asc');

    const [searchBy, setSearchBy] = useState(localStorage.getItem('parkingAddVehiclesSearchBy' + parkingId.toString()) || 'id');

    const [searchValue, setSearchValue] = useState(localStorage.getItem('parkingAddVehiclesSearchValue' + parkingId.toString()) || '');
    ...
    const togglePreviousPage = () => {
        ...
        history.push(`/parking/${parkingId}/add-vehicles?page=${page - 1}`)
    ...
    const addVehicle = async (id) => {
        ...
        try {
            const response = await fetch(`http://localhost:2810/parking/${parkingId}/addVehicle/${id}`, {
                method: 'PUT',
                headers: {
    ...
    const handleSortChange = (event) => {
        ...
        setSortBy(value);
        localStorage.setItem('parkingAddVehiclesSortBy' + parkingId.toString(), value);
    }
    ...
    const handleOrderChange = (event) => {
        const { value } = event.target;
        setOrder(value);
        localStorage.setItem('parkingAddVehiclesOrder' + parkingId.toString(), value);
    }

    const handleSearchChange = (event) => {
        const { value } = event.target;
        setSearchBy(value);
        localStorage.setItem('parkingAddVehiclesSearchBy' + parkingId.toString(), value);
    }

    const handleSearchValueChange = (event) => {
        const { value } = event.target;
        setSearchValue(value);
        localStorage.setItem('parkingAddVehiclesSearchValue' + parkingId.toString(), value);
    }

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:2810/vehicle/can-be-added`,{
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${localStorage.getItem('jwt')}`
                ...
                const afterRequestResponse = await fetch(`http://localhost:2810/vehicle/all-after-request?parkingId=${parkingId}`,{
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${localStorage.getItem('jwt')}`
                ...
                const bodyAfterRequest = await afterRequestResponse.json();
                console.log('Response:', body);
                console.log('Response Parking:', bodyParking);
                setClients(body);
                setParking(bodyParking);
                setAfterRequest(bodyAfterRequest);
    ...
    useEffect(() => {
        if ((page > totalPages && totalPages > 0) || page <= 0) {
            ...
            setPage(1);
            history.push(`/parking/${parkingId}/add-vehicles?page=1`)
        }
    }, [page, totalPages]);
    ...
    return (
        <div style={{display:"flex", flexDirection: "column", minHeight:"100vh"}}>
            ...
            <Container className="mt-5" style={{flex:1, marginBottom: '7vh'}}>
                <h2 className="mb-4 text-center">Add Vehicle in Parking {parking ? parking.name : ''}</h2>
                <h2 className="mb-3 text-center">You can add maximum {parking ? parking.maxCapacity - afterRequest.length : ''} vehicles</h2>
                    ...
                    {vehiclesForPage.map((client) => (
                        <tr key={client.id}>
                            ...
                            <td>
                                ...
                                <Button onClick={() => addVehicle(client.id)} className="btn btn-info mr-2">Add</Button>
                            </td>
                        </tr>
                    ))}
                    </tbody>
...
export default ParkingAddVehicles;
